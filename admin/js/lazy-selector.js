...
(function ($) {
  ...
  $.fn.lazySelector = function (options) {
    var defaults = {},
        ...
        $iconButton = $('<a class="icon" id="get-event-by-Id-' + options.selector + '" data-toggle="modal" data-target="#js_ls-modal" href="javascript:void(0)"/>');
    ...
    var $input = $(this),
			$modal = $('#js_ls-modal'),
      $form = $('#js_ls_form', $modal),
      $ul = $('#js_lazy-sel_list',$modal),
      $liFooter = $('#load-more-box'),
      $hr = $('#js_ls-top-hr',$modal),
    ...
    var keyword = $form.find('#ls_keyword'),
      defaultApiKey = 'pLOeuGq2JL05uEGrZG7DuGWu6sh2OnMz' , //apiKeyService.getApiExploreKey(),
    ...
    function addMarker(mapCenter) {
      var marker = new google.maps.Marker({ //Create a marker and set its position.
        map: map,
        position: mapCenter,
        icon: new google.maps.MarkerImage('../wp-content/plugins/ticketmaster/admin/img/pin-ic.svg',
    ...
    function modalHiddenListener(e) {
      // console.log('.on hidden.bs.modal' , selector);
      var indToRemove =[],
    ...
    /*$modal
      .on('shown.bs.modal', modalShowListener)
      .on('hidden.bs.modal', modalHiddenListener);*/

    $( '#countdown-config' )
      .delegate( '.modal.modal-common.fade.in', 'click', function ( e ) {
        if ( e.target.className === 'modal modal-common fade in' ) {
          $( this ).removeClass( 'in' );
          $('body').removeClass( 'modal-open' );
          modalHiddenListener();
        }
    ...
    $( '.modal-header .close' ,'#js_ls-modal').on( 'click', function ( e ) {
      $modal.removeClass( 'in' );
      $('body').removeClass( 'modal-open' );
      modalHiddenListener();
    } );
    $modal.on("keyup", keyEscListener);
    keyword.on("keyup", keyEscListener)
      .on("keyup", function (e) {
        var input = $(e.target);
        var form = $form.get(0);
    ...
    ;
    $btnGET.on("keyup", keyEscListener);
    ...
    return this.each(function () {
      ...
      function init(input) {
        ...
        $('#get-event-by-Id-' + options.selector + '')
          .on('click', changeModalTextListener)
          .on( 'click', function ( e ) {
            $modal.addClass( 'in' );
            $('body').addClass( 'modal-open' );
            $modal.css( 'display', 'block' );
            modalShowListener();
        } ).on("keyup", keyEscListener);
        tagsIds[selector] = [];
      }
    });
...
(function ($) {
  $(document).on('ready', function () {
    ...
    $('#w-attractionid').lazySelector({selector: 'attractions'});
    $('#w-venueid').lazySelector({selector: 'venues'});
  });
})(jQuery);
