...
(function ($) {
  ...
  $.fn.classificationSelector = function (options) {
    var defaults = {},
        ...
        $iconButton = $('<a class="icon" id="get-'+options.selector+'-'+options.use+'" href="javascript:void(0)" />');
    ...
    var keyword = $form.find('#keyword'),
      apikey =  $('#w-tm-api-key').val() || 'pLOeuGq2JL05uEGrZG7DuGWu6sh2OnMz',
    ...
    // $("#get-classifications-name").on( 'click', function ( e ) {
    //   console.log('click get-classifications-name');
    //   $( '#js_ls-modal-classification' ).addClass( 'in' );
    //   $( '#js_ls-modal-classification' ).css( 'display', 'block' );
    // } );

    $( '#countdown-config' )
      .delegate( '.modal.modal-common.fade.in', 'click', function ( e ) {
        if ( e.target.className === 'modal modal-common fade in' ) $( this )
          .removeClass( 'in' );
      } );
    ...
    return this.each(function (i) {
      ...
      function init(input) {
        var buttonIDSelector = '#get-'+options.selector+'-'+options.use;
...
/**
 * add classification selector to widgets
 */
(function ($) {
