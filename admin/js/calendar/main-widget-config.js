(
	function ( $ ) {
		...
		var changeState = function changeState( event ) {
			if ( ! event.target.name || event.target.name === "w-googleapikey" ) {
				return;
			}
			...
			if ( targetName === "w-tm-api-key" ) {
				document.querySelector( '[w-type="calendar"]' )
			...
			if ( targetName === "w-keyword" ) {
				...
				if ( document.getElementById( 'w-keyword' ).value == '' ) {
					...
					document.querySelector( '[w-type="calendar"]' )
					        .setAttribute( 'w-country', 'US' );
					document.querySelector( '[w-type="calendar"]' )
					        .setAttribute( 'w-latlong', '34.0390107,-118.2672801' );
				}
			}
			...
			if ( targetName === "w-postalcode" ) {
				...
				if ( document.getElementById( 'w-postalcode' ).value.length <= 3 ) {
					document.getElementById( 'w-postalcode' ).value = '90015';
				}
				;
			}
			...
			if ( targetName === "w-radius" ) {
				if ( document.getElementById( 'w-radius' ).value == '' ) {
					document.getElementById( 'w-radius' ).value = '5';
				}
			}
		...
		var resetWidget = function resetWidget( configForm ) {
			...
			configForm.find( "input[type='text'], input[type='number']" )
			          .each( function () {
				          var $self = $( this ),
					          data = $self.data(),
			...
			$( '.country-select .js_custom_select' )
				.removeClass( 'custom_select-opened' ); //reset custom select
		...
		function addCustomList( listWrapperElement, listWrapperId, activeVal ) {
			...
			//update list
				$placeholder = $( ".country-select" )
					.find( ".custom_select__placeholder" ),
			...
			$listOption.each( function () {
				...
				$ul.append( '<li class="custom_select__item ' + (
						activeVal === data.value ? 'custom_select__item-active' : ''
					) + '" data-value="' + data.value + '">' + $( this )
					            .text() + '</li>' );
			} );
		}
		...
		$('.widget__location span').on('click', function () {
			...
			document.querySelector('[w-type="calendar"]').removeAttribute('w-postalcode');
		...
		$('.widget__latlong span').on('click', function () {
			...
			document.querySelector('[w-type="clanedar"]').setAttribute('w-countrycode', widget.config.countrycode);
			document.querySelector('[w-type="calendar"]').setAttribute('w-postalcode', widget.config.postalcode);
